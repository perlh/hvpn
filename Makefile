...
build-linux-arm64:
	$(eval GOOS=linux)
	$(eval ARCH=arm64)
	CGO_ENABLED=0 GOOS=linux GOARCH=$(ARCH) $(GOBUILD) -o $(BIN_DIR)/$(BINARY_NAME)_$(GOOS)_$(ARCH)
...
build-windows-amd64:
	$(eval GOOS=windows)
	$(eval ARCH=amd64)
	CGO_ENABLED=0 GOOS=linux GOARCH=$(ARCH) $(GOBUILD) -o $(BIN_DIR)/$(BINARY_NAME)_$(GOOS)_$(ARCH).exe
